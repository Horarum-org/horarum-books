...
export function insertVariant(variant: Variant, db: Database): Promise<number> {
    return new Promise((resolve) => {
        db.serialize(() => {
            const stmt = db.prepare(`
                INSERT INTO variants (id, title, language, workRowId)
                SELECT ?, ?, ?, w.rowId
                FROM works w
                WHERE w.id = ?`);
            stmt.run(variant.id, variant.title, variant.language, variant.workId);
...
function getMaxNodeRowId(db: Database): Promise<number> {
    return new Promise<number>((resolve) => {
        db.serialize(() => {
            db.get(`SELECT max(rowId)
                    from nodes`, (_, row: object) => {
